...
# class UserViewSet(djoser_views.UserViewSet):
#     """Вьюсет пользователей с подписками."""
#
#     queryset = User.objects.all()
#     serializer_class = UserSerializer
#     pagination_class = PageNumberPagination
...
#     permission_classes = [IsAuthenticated | ReadOnly]
#
#     # Методы для подписок (перенесены из миксина)
#     def _get_subscription_data(self, request: Request, id: int) -> dict:
#         return {
#             'user': request.user,
#             'author_recipe': get_object_or_404(User, id=id)
#         }
...
#     def subscribe(self, request: Request, id: int):
#         data = self._get_subscription_data(request, id)
#         serializer = SubscriptionChangedSerializer(
#             data={key: obj.id for key, obj in data.items()},
#             context={'request': request}
#         )
...
#     def unsubscribe(self, request: Request, id: int):
#         return object_delete(
#             data=self._get_subscription_data(request, id),
...
#         )
#
#     # Методы для аватара (оставлены без изменений)
#     @action(['PUT'], detail=False, url_path='me/avatar', permission_classes=[IsAuthenticated])
#     def avatar(self, request: Request, *args, **kwargs):
#         if 'avatar' not in request.data:
#             return response.Response(
#                 {'avatar': 'Отсутствует изображение'},
